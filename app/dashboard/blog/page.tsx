...
export default function DashboardBlogListPage() {
  ...
  const canView = hasAnyPermission(session?.user?.permissions ?? [], ['blog.view'])
  ...
  if (status === 'loading') {
    return (
      <div className="max-w-4xl mx-auto pt-12 text-center">
        <p className="font-sans text-[14px] text-taupe">Loading blog posts…</p>
      </div>
    )
  }

  if (!canView) {
    return <DashboardAccessDenied message="You do not have permission to view blog posts." />
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <select value={topic} onChange={e => setTopic(e.target.value)}
            className={`${inputCls} w-full sm:w-[180px]`} style={{ borderWidth: '0.5px' }}>
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="border-b border-light-gray bg-light-gray/10" style={{ borderWidth: '0 0 0.5px 0' }}>
              {['Title', 'Topic', 'Author', 'Published', 'Status', 'Actions'].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="p-16 text-center">
                <p className="font-serif text-[20px] text-charcoal mb-2">Failed to load posts</p>
                <p className="font-sans text-[14px] text-taupe">Check your database connection and try again.</p>
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={6} className="p-16 text-center">
                <p className="font-serif text-[20px] text-charcoal mb-2">No posts found</p>
                <p className="font-sans text-[14px] text-taupe">Try adjusting your search or topic filter.</p>
              </td></tr>
            ) : filtered.map((p, i) => (
              <tr key={p.id}
                ...
                <td className="p-4">
                  <span className="px-2.5 py-1 bg-light-gray/40 text-charcoal font-sans text-[12px] rounded-sm">{p.topic}</span>
                </td>
                <td className="p-4 font-sans text-[14px] text-taupe">{p.author}</td>
                <td className="p-4 font-sans text-[14px] text-taupe">{formatDate(p.publishedAt)}</td>
                <td className="p-4">
                  <StatusBadge status={p.isPublished ? 'Published' : 'Draft'} variant={p.isPublished ? 'gold' : 'gray'} />
                </td>
                <td className="p-4 text-right space-x-4">
                  <Link href={`/dashboard/blog/edit/${p.id}`}
