...
export default async function BlogPostPage({
  ...
  const fmt=(d:Date|string|null)=>d?new Date(d).toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'}):''
      ...
      <article className="max-w-[740px] mx-auto px-4 md:px-8 py-12 md:py-16">
        ...
        <div className="space-y-6">
          {paras.map((para:string,i:number)=>
            para.startsWith('## ')
              ? <h2 key={i} className="font-serif text-[24px] text-charcoal leading-[1.3] mt-10 mb-2">{para.replace('## ','')}</h2>
              : <p key={i} className="font-sans text-[15px] text-taupe leading-[1.8]">{para}</p>
          )}
        </div>
        {post.pullQuote&&(
